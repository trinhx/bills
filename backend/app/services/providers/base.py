...
def with_retry(
    ...
    retry_exceptions: tuple = (requests.exceptions.RequestException,),
):
    """
    ...
    ``retry_exceptions`` controls which exception types trigger a retry. By
    default only ``requests.exceptions.RequestException`` is retried, which
    preserves existing behaviour. Providers that wrap non-requests libraries
    (e.g. ``yfinance``) should pass a broader tuple so that ``JSONDecodeError``
    or other library-internal errors don't bypass the retry logic.
    """
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            while attempts < max_attempts:
                ...
                try:
                    ...
                    # Extract HTTP status if this is a requests exception,
                    # otherwise treat as a generic transient failure.
                    ...
                    # Determine retryability. Non-HTTP exceptions always retry
                    # (we assume the caller only included transient types in
                    # ``retry_exceptions``). HTTP 4xx (except 429) is terminal.
                    ...
                    if response is not None and getattr(response, "headers", None):
                        ...
                        if header:
                            try:
                                ...
                                pass  # sometimes it's an HTTP date, fall back to backoff
                    ...
                    if attempts == max_attempts:
                        raise NetworkException(
                            str(e), status_code=status_code, retry_after=retry_after
                        ) from e
                    ...
                    logger.warning(
                        f"Request failed ({type(e).__name__} status={status_code}). "
                        f"Retrying in {delay:.2f}s (Attempt {attempts}/{max_attempts})"
                    )
                    time.sleep(delay)
