...
def mock_requests_get(url, *args, **kwargs):
    if "cage.dla.mil/Search/Results" in url:
        if "CAGE_HAPPY" in url:
            return MockResponse(
                200, text='<a href="/Search/Details?id=happy">Details</a>'
            )
        elif "CAGE_RETRY" in url:
            return MockResponse(
                200, text='<a href="/Search/Details?id=retry">Details</a>'
            )
        elif "CAGE_NOPARENT" in url:
            return MockResponse(
                200, text='<a href="/Search/Details?id=noparent">Details</a>'
            )
        ...
        if "CAGE_HAPPY" in url or "HAPPY" in url:
            return MockResponse(
                ...
                json_data={
                    "quoteSummary": {
                        "result": [{"price": {"marketCap": {"raw": 1000000000.0}}}]
                    }
                },
            )
        ...
        return MockResponse(
            ...
            json_data={
                "quoteSummary": {
                    "result": [{"price": {"marketCap": {"raw": 500000000.0}}}]
                }
            },
        )
...
@patch(
    "backend.app.services.providers.base.time.sleep"
)  # Don't actually sleep in tests
@patch(
    "backend.app.services.providers.openfigi.OPENFIGI_RATE_LIMITER.wait"
)  # Skips proactive waiting
@patch(
    "backend.scripts.enrich.CAGE_COOKIES",
    {"PHPSESS": "mock", "__RequestVerificationToken": "mock", "agree": "True"},
)
@patch(
    "backend.scripts.enrich.validate_cage_credentials",
    lambda: None,  # skip real credential check in integration tests
)
def test_end_to_end_pipeline(
    ...
    with (
        patch(
            "backend.scripts.enrich.get_cleaned_conn", side_effect=mock_get_cleaned_conn
        ),
        patch("backend.scripts.enrich.get_cache_conn", side_effect=mock_get_cache_conn),
        patch(
            "backend.scripts.themes.get_cleaned_conn", side_effect=mock_get_cleaned_conn
        ),
        patch(
            "backend.scripts.signals.get_cleaned_conn",
            side_effect=mock_get_cleaned_conn,
        ),
        ...
        def mock_execute(self, query, *args, **kwargs):
            if "ATTACH 'backend/data/cache/cache.duckdb'" in query:
                return real_execute(
                    self, f"ATTACH '{cache_path}' AS cache;", *args, **kwargs
                )
        ...
        with patch.object(duckdb.DuckDBPyConnection, "execute", new=mock_execute):
            ...
            openfigi_rows = cache_conn.execute(
                "SELECT * FROM cache.cache_openfigi_ticker"
            ).fetchall()
            failures = cache_conn.execute(
                "SELECT provider, key, error_type, message, http_status, retry_after_seconds FROM cache.cache_failures"
            ).fetchall()
            ...
            failures = cache_conn.execute(
                "SELECT provider, key, http_status, retry_after_seconds FROM cache.cache_failures WHERE http_status=429"
            ).fetchall()
            assert len(failures) > 0, (
                "Pipeline should have caught the 429 and logged it to failures table."
            )
            for fail in failures:
                assert fail[3] > 0, (
                    "429 Failure must have a retry_after_seconds > 0 to prevent infinite 0-sec retry loop."
                )
    ...
    assert len(df) >= 3, (
        "All 3 initial rows should make it to the final output, and may be multiplied by 1-to-many lookup joins."
    )
    ...
    # Schema contract: the final CSV columns must equal the fields declared
    # in SignalsAward. Regression guards for cache-metadata leakage
    # (result_status / last_verified) and duplicated naics_description_1.
    from dataclasses import fields
    from backend.models.contracts import SignalsAward

    expected_columns = {f.name for f in fields(SignalsAward)}
    actual_columns = set(df.columns)

    unexpected = actual_columns - expected_columns
    missing = expected_columns - actual_columns
    assert not unexpected, f"Final CSV has unexpected columns: {sorted(unexpected)}"
    assert not missing, f"Final CSV is missing expected columns: {sorted(missing)}"

    # Explicit regression guards for issues we previously shipped.
    assert "result_status" not in df.columns
    assert "last_verified" not in df.columns
    assert "naics_description_1" not in df.columns

    # M1.5 P2-3 guard: enriched_awards must store string-domain columns
    # as VARCHAR even when every value is NULL (mock pipeline routinely
    # hits this case for market_cap_quality on rows without market_cap).
    conn = duckdb.connect(db_path, read_only=True)
    try:
        enriched_schema = conn.execute("DESCRIBE enriched_awards").df()
        col_types = dict(
            zip(enriched_schema["column_name"], enriched_schema["column_type"])
        )
        for must_be_varchar in (
            "market_cap_quality",
            "transaction_type",
            "ticker",
            "sector",
            ...
            assert col_types.get(must_be_varchar) == "VARCHAR", (
                f"enriched_awards.{must_be_varchar} expected VARCHAR, "
                f"got {col_types.get(must_be_varchar)}"
            )
    finally:
        conn.close()
